        ...
        if os.path.exists('logs/usage_log.txt'):
            ...
            <head>
                ...
                <style>
                    ...
                    .download-btn {{ background: #E57C23; color: white; padding: 10px 20px; text-decoration: none; 
                                     border-radius: 5px; display: inline-block; margin: 10px 10px 20px 0; }}
                    .stats {{ background: #FFF9F0; padding: 15px; border-radius: 8px; margin: 20px 0; }}
                </style>
            </head>
            <body>
                <div class="container">
                    <h1>📊 Usage Logs - Aaj Ki Pakayein</h1>
                    <a href="/download_logs" class="download-btn">⬇️ Download Logs</a>
                    <a href="/view_feedback" class="download-btn" style="background: #4CAF50;">💬 View Feedback</a>
                    ...
                    <h2>Log Entries:</h2>
                    <pre>{content if content else "No logs yet"}</pre>
                </div>
            </body>
            </html>
            '''
            ...
            return '<h1>No logs found yet</h1><a href="/">Back to App</a>'
    except Exception as e:
        return f'<h1>Error reading logs: {str(e)}</h1><a href="/">Back to App</a>'

@app.route('/view_feedback')
def view_feedback():
    """View feedback in browser"""
    try:
        if os.path.exists('logs/feedback.txt'):
            ...
            <head>
                ...
                <style>
                    ...
                    .download-btn {{ background: #4CAF50; color: white; padding: 10px 20px; text-decoration: none; 
                                     border-radius: 5px; display: inline-block; margin: 10px 10px 20px 0; }}
            ...
            <body>
                <div class="container">
                    <h1>💬 User Feedback - Aaj Ki Pakayein</h1>
                    <a href="/download_feedback" class="download-btn">⬇️ Download Feedback</a>
                    <a href="/view_logs" class="download-btn" style="background: #E57C23;">📊 View Logs</a>
                    <a href="/" class="download-btn" style="background: #666;">🏠 Back to App</a>
                    
                    <div class="stats">
                        <strong>Total Feedback Received:</strong> {feedback_count}
                    </div>
                    
                    <h2>Feedback Entries:</h2>
                    <pre>{content if content else "No feedback yet"}</pre>
                </div>
            </body>
            </html>
            '''
...
def download_logs():
    ...
    try:
        if os.path.exists('logs/usage_log.txt'):
            return send_file('logs/usage_log.txt', 
                           as_attachment=True, 
                           download_name=f'usage_logs_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt')
        else:
...
def download_feedback():
    ...
    try:
        if os.path.exists('logs/feedback.txt'):
            return send_file('logs/feedback.txt', 
                           as_attachment=True, 
                           download_name=f'feedback_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt')
        else:
